...
def pre_process_bool(key, value, village_id=None):
    if village_id:
        if value:
            return (
                '<button class="btn btn-sm btn-block btn-success" data-village-id="%s" data-type-option="%s" data-type="toggle">Enabled</button>'
                % (village_id, key)
            )
        else:
            return (
                '<button class="btn btn-sm btn-block btn-danger" data-village-id="%s" data-type-option="%s" data-type="toggle">Disabled</button>'
                % (village_id, key)
            )
    if value:
        return (
            '<button class="btn btn-sm btn-block btn-success" data-type-option="%s" data-type="toggle">Enabled</button>'
            % key
        )
    else:
        return (
            '<button class="btn btn-sm btn-block btn-danger" data-type-option="%s" data-type="toggle">Disabled</button>'
            % key
        )
...
def preprocess_select(key, value, templates, village_id=None):
    output = (
        '<select data-type-option="%s" data-type="select" class="form-control">' % key
    )
    if village_id:
        output = (
            '<select data-type-option="%s" data-village-id="%s" data-type="select" class="form-control">'
            % (key, village_id)
        )
...
def pre_process_string(key, value, village_id=None):
    ...
    if village_id:
        return (
            '<input type="text" class="form-control" data-village-id="%s" data-type="text" value="%s" data-type-option="%s" />'
            % (village_id, value, key)
        )
    else:
        return (
            '<input type="text" class="form-control" data-type="text" value="%s" data-type-option="%s" />'
            % (value, key)
        )
...
def pre_process_number(key, value, village_id=None):
    if village_id:
        return (
            '<input type="number" data-type="number" class="form-control" data-village-id="%s" value="%s" data-type-option="%s" />'
            % (village_id, value, key)
        )
    return (
        '<input type="number" data-type="number" class="form-control" value="%s" data-type-option="%s" />'
        % (value, key)
    )
...
def pre_process_list(key, value, village_id=None):
    if village_id:
        return (
            '<input type="text" data-type="list" class="form-control" data-village-id="%s" value="%s" data-type-option="%s" />'
            % (village_id, ", ".join(value), key)
        )
    return (
        '<input type="number" data-type="list" class="form-control" value="%s" data-type-option="%s" />'
        % (", ".join(value), key)
    )
...
def pre_process_village_config(village_id):
    ...
    for parameter in config:
        ...
        if type(value) == bool:
            config_data += "%s %s" % (
                fancy(kvp),
                pre_process_bool(kvp, value, village_id),
            )
        if type(value) == str:
            config_data += "%s %s" % (
                fancy(kvp),
                pre_process_string(kvp, value, village_id),
            )
        if type(value) == list:
            config_data += "%s %s" % (
                fancy(kvp),
                pre_process_list(kvp, value, village_id),
            )
        if type(value) == int or type(value) == float:
            config_data += "%s %s" % (
                fancy(kvp),
                pre_process_number(kvp, value, village_id),
            )
    return config_data
